...
impl GraphqlClient {
    ...
    pub async fn query_with_variables<T: for<'de> Deserialize<'de>, V: serde::Serialize>(
        &self,
        query: &str,
        variables: Option<V>,
    ) -> Result<T, GraphqlRequestError> {
        ...
        loop {
            ...
            let response = match response_result {
                Ok(r) => r,
                Err(e) if is_transient_network_error(&e) && retries < max_retries => {
                    retries += 1;
                    tokio::time::sleep(Duration::from_millis(DEFAULT_RETRY_DELAY_MS)).await;
                    continue;
                }
                Err(e) => {
                    return Err(GraphqlRequestError::Network(e.to_string()));
                }
            };
            ...
            let text = response.text().await.map_err(|e| GraphqlRequestError::Network(e.to_string()))?;
            let full_response: GraphqlResponse<Value> = serde_json::from_str(&text)
                .map_err(|e| GraphqlRequestError::Parse(e.to_string(), text.clone()))?;
            ...
            if let Some(errors) = &full_response.errors {
                if full_response.data.is_none() && status.is_server_error() {
                    ...
                    return Err(GraphqlRequestError::ApiError(msg, status.as_u16()));
                }
                if full_response.data.is_none() && !status.is_server_error() {
            ...
            if let Some(data) = full_response.data {
                if status.is_success() {
                    ...
                    let result: T = serde_json::from_value(data)
                        .map_err(|e| GraphqlRequestError::Parse(e.to_string(), text))?;
                    return Ok(result);
                }
            }

            if status.is_client_error() && retries < max_retries {
                retries += 1;
                tokio::time::sleep(Duration::from_millis(DEFAULT_RETRY_DELAY_MS)).await;
                continue;
            }
...
async fn wait_for_rate_limit_restore(cost: &GraphqlCost) {
    if let (Some(actual), Some(restore_rate)) = (cost.actual_query_cost, cost.throttle_status.as_ref().and_then(|t| t.restore_rate)) {
...
fn extract_error_messages(errors: &[GraphqlError]) -> String {
    ...
    for error in errors {
        if let Some(app_errors) = error.extensions.as_ref().and_then(|ext| ext.app_errors.as_ref()) {
...
mod tests {
    ...
    #[tokio::test]
    async fn is_transient_network_error_does_not_panic() {
        // Verify the function doesn't panic on any real reqwest error
        let result = reqwest::Client::builder()
            ...
            .await;
        if let Err(e) = result {
            let _ = is_transient_network_error(&e);
        }
    }
    ...
    fn graphql_request_error_display_api() {
        ...
        assert_eq!(format!("{err}"), "GraphQL API error (HTTP 400): bad request");
    }
    ...
    async fn query_429_returns_api_error_after_retries_exhausted() {
        ...
        // Always return 429 - client should eventually give up
        Mock::given(method("POST"))
            .and(path("/"))
        ...
        let client = GraphqlClient::new(mock_server.uri(), None);
        let result: Result<serde_json::Value, _> = client.query("{ value }").await;
        // After 10 retries, should get an error (either ApiError or Parse)
    ...
    async fn client_with_custom_reqwest_client() {
        ...
        // Should not panic: the client is constructed with a custom reqwest client
    }
...
impl From<GraphqlRequestError> for FatalError {
    fn from(e: GraphqlRequestError) -> Self {
        match e {
            GraphqlRequestError::ApiError(msg, status) if status < 500 => {
                abort_error(msg, None::<String>, vec![])
            }
            ...
            GraphqlRequestError::Parse(msg, _) => {
                abort_error(msg, None::<String>, vec![])
            }
        }
    }
